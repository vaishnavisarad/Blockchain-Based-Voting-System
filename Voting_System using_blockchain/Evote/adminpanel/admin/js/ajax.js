...
// Add Vote 
$(document).on("submit","#addVoteFrm" , function(){
  $.post("query/addVoteExe.php", $(this).serialize() , function(data){
    ...
    if(data.res == "noDisplayLimit")
   {
     ...
     else if(data.res == "exist")
    {
      Swal.fire(
        'Already Exist',
        data.VoteTitle.toUpperCase() + '<br>Already Exist',
        'error'
      )
    }
    else if(data.res == "success")
    {
      Swal.fire(
        'Success',
        data.VoteTitle.toUpperCase() + '<br>Successfully Added',
        'success'
      )
          $('#addVoteFrm')[0].reset();
          $('#course_name').val("");
...
// Update Vote 
$(document).on("submit","#updateVoteFrm" , function(){
  $.post("query/updateVoteExe.php", $(this).serialize() , function(data){
    if(data.res == "success")
    {
      Swal.fire(
          'Update Successfully',
...
$(document).on("submit","#addQuestionFrm" , function(){
  $.post("query/addQuestionExe.php", $(this).serialize() , function(data){
    if(data.res == "exist")
    {
      Swal.fire(
          'Already Exist',
          data.msg + ' question <br>already exist in this Vote',
          'error'
       )
    }
    else if(data.res == "success")
...
// Add Voteinee
$(document).on("submit","#addVoteineeFrm" , function(){
  $.post("query/addVoteineeExe.php", $(this).serialize() , function(data){
...
// Update Voteinee
$(document).on("submit","#updateVoteineeFrm" , function(){
  $.post("query/updateVoteineeExe.php", $(this).serialize() , function(data){
     if(data.res == "success")
     {
        Swal.fire(
            'Success',
